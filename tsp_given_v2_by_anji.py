...
#     SWP02 (Tuesday 7,8)
################################################################################
STUDENT_ID = '2014110045' # Edit this
STUDENT_NAME = '박은지' # Edit this
STUDENT_NICKNAME = '박은지' # Edit this
...
class TSP:
    ...
    def make_random_path(self):
        ...
        result = [node[0] for node in self.nodes] # self.nodes라는 list의 각 node에서 0번째 element를 가지고 온 것=result

        random.shuffle(result) # 경로를 뒤섞음 (shuffle)

        result.append( result[0] ) # 첫 도시 인덱스를 경로의 맨 뒤에 추가하여 경로를 닫음

        self.set_new_path( result, 'Random Algorithm' ) # 완성된 임시 경로를 현재 경로로 저장


    # Greedy Algorithm
    def make_greedy_path(self):

        pool = list(range(len(self.nodes))) # 모든 도시의 인덱스를 가지고 있는 pool 리스트 생성

        result = [] # 비어있는 임시 경로(result) 준비
        
        result.append(pool.pop(0)) # pool에서 꺼낸 도시 인덱스 하나를 result에 추가
        ...
        while len( pool ) > 0:
            ...
            for i in range(len(pool)):
                ...
                n1 = result[len(result)-1] #result의 길이를 알아야 하므로 len을 쓴다

                # pool 안에 남아있는 도시의 인덱스 중 하나를 가져옴
                n2 = pool[i]
                ...
                if min_dist is None or dist < min_dist:
                    min_dist = dist
                    min_index = i                 # min_dist 와 min_index를 갱신
    ...
    def make_2opt_path(self):
        ...
        for i in range(100):
            ...
            if self.calc_path_length(result) < self.calc_path_length(self.path):
                # 변경된 경로를 현재 경로로 저장
